...
public class Avion {
     ...
     *                  CENTRAL, PASILLO}, o si clase = CLASE_EJECUTIVA entonces
     *                  ubicaci�n pertenece {VENTANA, PASILLO}
    ...
    public int darDuracionTotalPeliculas() {
        ...
        for (int i = 0; i < SILLAS_ECONOMICAS; i++) {
            if (sillasEconomicas[i].sillaAsignada()) {
                duracionTotal += sillasEconomicas[i].darPasajero().darDuracionTotalPeliculas();
            }
        }
        for (int i = 0; i < SILLAS_EJECUTIVAS; i++) {
            if (sillasEjecutivas[i].sillaAsignada()) {
                duracionTotal += sillasEjecutivas[i].darPasajero().darDuracionTotalPeliculas();
                ;
            }
        }
